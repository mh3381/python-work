import unittest
import polygonAreaCalc


class UnitTests(unittest.TestCase):
    def setUp(self):
        self.rect = polygonAreaCalc.Rectangle(3, 6)
        self.sq = polygonAreaCalc.Square(5)

    def test_subclass(self):
        actual = issubclass(polygonAreaCalc.Square, polygonAreaCalc.Rectangle)
        expected = True
        self.assertEqual(actual, expected, 'Expected Square class to be a subclass of the Rectangle class.')

    def test_distinct_classes(self):
        actual = polygonAreaCalc.Square is not polygonAreaCalc.Rectangle
        expected = True
    ...
    def test_square_is_square_and_rectangle(self):
        actual = isinstance(self.sq, polygonAreaCalc.Square) and isinstance(self.sq, polygonAreaCalc.Rectangle)
        expected = True
    ...
    def test_get_amount_inside_two_rectangles(self):
        rect2 = polygonAreaCalc.Rectangle(4, 8)
    ...
    def test_get_amount_inside_none(self):
        rect2 = polygonAreaCalc.Rectangle(2, 3)
        actual = rect2.get_amount_inside(self.rect)
        expected = 0
